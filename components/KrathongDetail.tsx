...
interface KrathongDetailProps {
  krathong: Krathong;
  currentUserEmail?: string | null; // เพิ่ม prop นี้เพื่อตรวจสอบผู้ใช้ปัจจุบัน
}

const KrathongDetail: React.FC<KrathongDetailProps> = ({ krathong, currentUserEmail }) => {
  // ตรวจสอบว่าผู้ใช้ปัจจุบันเป็นสมาชิกในทีมนี้หรือไม่
    ...
    <div className="space-y-8 max-h-[80vh] overflow-y-auto pr-4">
      ...
      <div className="text-center">
        ...
        <div className="flex items-center justify-center space-x-4">
          <div className="bg-slate-700/50 px-4 py-2 rounded-full border border-amber-400/30">
            ...
            <span className="text-slate-300 ml-2">คะแนน</span>
          </div>
          {isCurrentUserInTeam && (
      ...
      <div className="bg-slate-800/50 rounded-xl p-6 border border-slate-700">
        <h4 className="text-xl font-bold text-slate-300 mb-4 flex items-center">
          ...
          สมาชิกในทีม ({krathong.members.length} คน)
        </h4>
